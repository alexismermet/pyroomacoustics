...
\begin{itemize}
	...
	\item If there is only noise at a given time and frequency, the gain filter has value $ G_{min} $.\todo{why $ G_{min} $ and not 0?}
...
\todo{refer to block diagram of STFT}
\begin{figure}
	\centering
...
\end{equation}
with $ w_{m} $ corresponding to the weights, used to improve the quality of the recording for the $ m^{th} $ signal $ x_{m}  $ and $\Delta_{m}$ corresponding to the delay chosen to maximize the array's sensitivity to waves propagating from a particular direction.\todo{I still think it's better to move the SNR computation for multiple microphone here so that you only have to refer back to the idea of SNR and then it makes sense to show the SNR for beamforming here after you have defined what beamforming is}

\chapter{Implementation}
\section{The GoogleSpeechCommands Dataset}
\hspace*{0.6cm}
The ``GoogleSpeechCommands" dataset wrapper was created as a subclass of the ``Dataset'' class that was already implemented in \textit{pyroomacoustics}. This class will load Google's Speech Commands Dataset in a structure that is convenient to be processed. It has four main attributes:\todo{perhaps it would be better to write this as a list as you've done for the functions below} the directory where the dataset is located, the ``basedir". A dictionary whose keys are word in the dataset. The values are the number of occurrences for that particular word in a dictionary called ``size\_by\_samples". A list of subdirectories in ``basedir", where each sound type is the name of a subdirectory, called ``subdirs". And finally ``classes", the list of all sounds, which is the same as the keys of ``size\_by\_samples".\\
\\
\hspace*{0.6cm}
...
\end{enumerate}
\hspace*{0.6cm}
Now that we talk about the wrapper, we need to present also the ``GoogleSample" class that is inheriting from the class ``AudioSample" created beforehand in \textit{pyroomacoustics}. This class allows the user to create an audio object to print it in a nice way, plot the corresponding spectrogram, and listen to the file using the \emph{sounddevice} library.\todo{add reference to this}

\section{How to label a file?}
\label{sec:label_file}
\hspace*{0.6cm}
In this section, we are going to see how a user can label a file (following the example script available on \textit{pyroomacoustics}\todo{I think you should make it clear that it is available on \emph{your} fork of pyroomacoustics so you should provide the link to this at one point} called ``how\_to\_label\_a\_file"). This example uses the ``GoogleSpeechCommand'' dataset and also the graph we obtained by training TensorFlow neural network.
...
\begin{enumerate}
	\tightlist
	\item \texttt{load\_graph},\todo{Id recommend using this font to talk about function/variables of code} that is loading the graph used to label sounds.
...
\end{enumerate}
\hspace*{0.6cm}
In the script, the user needs to specify his label file and his graph file. In our example, he can choose one of the word from the list we saw in Section ~\ref{sec:google} and then label it using the \texttt{label\_wav} function in the following way:\todo{it would be better to use the lstlistings package for code snippets}
\begin{figure}[h!]
	\centering
...
Here \texttt{destination}\todo{change to this style for variables/functions} represents the directory in which the file to label is kept, labels\_file the label file, graph\_file the graph obtained from TensorFlow and finally word is the sound you except to obtain with this wav file.

\todo{emphasize that this is the core part of testing our algorithms! ie quantifying the speech recognition of the algorithms we will be testing later}
...
\end{enumerate}
We will only talk about the second one since the first function is just a special case of the second function. It can be done with the functions taking care of multiple microphones case (but this is less natural to use it in this case).\todo{why not get rename modify\_input\_wav\_multiple\_mics to modify\_input\_wav and get rid of the extra/unnecessary function?}
\begin{figure}[h!]
	\centering
	...
	\label{fig:rapport9}
\end{figure}\todo{this figure is too small to read...another reason why lstlistings would be nice}
...
\hspace*{0.6cm}
You can see an example of how to use this function in \textit{pyroomacoustics}\todo{in your fork}. It is called ``how\_to\_synthesize\_a\_signal". In this example, for a word from the ``GoogleSpeechCommands'' dataset (``no") and an SNR of 20, we obtained the following new noisy signal:
\begin{figure}[h!]
	\centering
	...
	\label{fig:rapport11}
\end{figure}\todo{make sure both original and noisy are normalize to the same maximum so that the comparison is more appropriate. I have the feeling whats why the leftmost is predominantly yellow}
...
\subsection{SCNR}
You can see how we implemented it in figure 3.8 however we will explain how we did it.\todo{I would remove large code snippets like this and break it into smaller ones while you describe it}\\
...
Even though we have not implemented the algorithm itself, we have used the DAS algorithm in a really specific way as you can see in figure 3.9.\todo{not super clear... I would say that you combined the ``synthesizing noise signals'' script with the already implemented beamforming to simulate the whole process. and then did something like the label file script to get the performance. a block diagram would be great!}
\begin{figure}[h!]
	\centering
...
We can conclude that the SNCR is a good algorithm for cleaning the data before giving it to a classifier. The improvement for low SNR are always close or above 20$\%$. But we can't say this is perfect. Even if the algorithm is quick, we can see that we don't achieve more that 50$\%$ of correctness in a majority of the case  for low SNR. This is not that good if we want  to use this algorithm for a vocal recognition system.\todo{did you get the performance for the other words? perhaps you can put it in a table}
\section{Analyse improvement of Beamforming}
\hspace*{0.6cm}
Now we will look at the efficiency of our DAS algorithm.\todo{what is size of array? could also be nice to plot the room with source and noise and the beampatterns as I did in the example script I created} We use the script ``analyze\_improvement\_of\_beamforming\_fulldata" available in \textit{pyroomacoustics}. We ran it for subset of size 25 in the GoogleSpeechCommands dataset and for all the word that the TenserFlow neural network can recognize. This means we have created 1750 samples per word and 25 samples per SNR for a given word. We obtained the following result:\\
\begin{figure}[h!]
	\centering
	\begin{subfigure}{.5\textwidth}
		\centering
