...
if __name__ == '__main__':
    ...
    t0 = 1./(16000*np.pi*1e-2)  # starting time function of sinc decay in RIR response
    ...
    mics_array = np.array([[1.92,1.5],[2.,1.5],[2.08,1.5]])
    ...
    print(score_adder_average)
    #######################################################################################

    #we use now beamforming RAKE MVDR

    fs_s, audio_anechoic = wavfile.read(speech_file_location)
    fs_n, noise_anechoic = wavfile.read(noise_file_location)
